...
master = Tk()
display = Entry(master, width=5, justify="right", bd=0, bg="black")

master.title("Calculator")


#  __INIT___ will allow us to create our objects for the Class!
...
# this is the method that helps ypu add the buttons
# Index is the number 
...
plus  = Button(master, text="+",command=lambda: calc.numb_butt(0))
minus = Button(master, text="-",command=lambda: calc.numb_butt(1))
times = Button(master, text="*",command=lambda: calc.numb_butt(2))
divide = Button(master, text="/",command=lambda: calc.numb_butt(3))
